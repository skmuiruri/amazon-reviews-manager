...
object AmazonReviewManagerSpec extends DefaultRunnableSpec {
  ...
  override def spec = suite("AmazonReviewManagerSpec")(
    testM("Return the correct top product ratings") {
      ...
      val o: ZIO[Any, Throwable, Vector[payload.ResponseData]] = AmazonReviewManager.findTopProducts(path, params).provideLayer(dependencies)
      for {
        res: List[payload.ResponseData] <- o.map(_.toList)
      } yield assert(res)(equalTo(List(ResponseData("B000JQ0JNS", 4.5), ResponseData("B000NI7RW8", 3.6666666666666665))
      ))
    }
  )
}
